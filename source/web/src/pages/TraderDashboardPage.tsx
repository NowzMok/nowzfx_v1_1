...
import { ChartTabsEnhanced } from '../components/ChartTabsEnhanced'
import { confirmToast, notify } from '../lib/notify'
import { t, type Language } from '../i18n/translations'
import { LogOut, Loader2, Eye, EyeOff, Copy, Check, RefreshCw } from 'lucide-react'
...
export function TraderDashboardPage({
  ...
  const [syncingPositions, setSyncingPositions] = useState<boolean>(false)
  ...
  const handleSyncPositions = async () => {
    ...
    try {
      const result = await api.syncPositions(selectedTraderId)
      // 修复返回类型：result 可能只包含 message，没有 count
      const count = (result as any).count || 0
      const message = result.message || (language === 'zh' ? '同步完成' : 'Sync completed')
      
      notify.success(
        language === 'zh'
          ? `同步成功：${message} ${count > 0 ? `(找到 ${count} 个持仓)` : ''}`
          : `Sync successful: ${message} ${count > 0 ? `(Found ${count} positions)` : ''}`
      )
      // 刷新数据
      await Promise.all([
        mutate(`positions-${selectedTraderId}`),
  ...
  const handleSyncOrders = async () => {
    ...
    try {
      const result = await api.syncOrders(selectedTraderId)
      // 修复返回类型：result 可能只包含 message，没有 total/filled
      const total = (result as any).total || 0
      const filled = (result as any).filled || 0
      const message = result.message || (language === 'zh' ? '同步完成' : 'Sync completed')
      
      notify.success(
        language === 'zh'
          ? `同步成功：${message} ${total > 0 ? `(总订单 ${total}，已成交 ${filled})` : ''}`
          : `Sync successful: ${message} ${total > 0 ? `(Total ${total}, Filled ${filled})` : ''}`
      )
      // 刷新数据
      mutate(`orders-${selectedTraderId}`)
    } catch (err: unknown) {
      const errorMsg =
        err instanceof Error
          ? err.message
    ...
    return (
      <DeepVoidBackground className="min-h-screen pb-12" disableAnimation>
        <div className="w-full px-4 md:px-8 pt-24">
          <div className="nofx-glass p-6 text-center">
            <div className="text-2xl mb-2 text-nofx-text-main">
              {language === 'zh' ? '未选择交易员' : 'No trader selected'}
            </div>
            <div className="text-nofx-text-muted">
              {language === 'zh'
                ? '请先在配置页选择或创建交易员'
                : 'Please select or create a trader in the config page'}
            </div>
            <div className="mt-4">
              <button
                className="px-4 py-2 rounded bg-nofx-gold/20 border border-nofx-gold/30 text-nofx-text-main hover:bg-nofx-gold/30 transition"
                onClick={onNavigateToTraders}
              >
                {language === 'zh' ? '前往配置页' : 'Go to Config'}
              </button>
            </div>
          </div>
        </div>
  ...
  return (
    <DeepVoidBackground className="min-h-screen pb-12" disableAnimation>
      <div className="w-full px-4 md:px-8 relative z-10 pt-6">
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <h2 className="text-xl font-bold flex items-center gap-2 text-nofx-text-main">
                  ...
                  {language === 'zh' ? '待执行订单' : 'Pending Orders'}
                </h2>
                <button
                  type="button"
                  onClick={handleSyncOrders}
                  disabled={syncingOrders}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded text-xs font-semibold transition-all hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed bg-orange-500/10 text-orange-400 border border-orange-500/30 hover:bg-orange-500/20 hover:border-orange-500/50"
                  title={language === 'zh' ? '从交易所同步最新的已成交订单数据' : 'Sync filled orders from exchange'}
                >
                  {syncingOrders ? (
                    <>
                    ...
                    <>
                      ...
                      {language === 'zh' ? '同步订单' : 'Sync Orders'}
                    </>
                  )}
                </button>
              </div>
              <PendingOrdersTable traderId={selectedTraderId} />
            </div>

            {/* TP/SL Tracking Panel */}
            <div
              className="nofx-glass p-5 animate-slide-in mb-6"
              style={{ animationDelay: '0.15s' }}
            >
              <TPSLPanel traderId={selectedTraderId} />
            </div>
          </>
        )}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          {/* Left: Chart Tabs (Equity / K-line) */}
          <div
            ref={chartSectionRef}
          ...
          >
            {selectedTrader?.trader_id ? (
              ...
              <div className="nofx-glass p-6 text-nofx-text-muted text-sm">
                {t('loading', language)}
              </div>
            )}
          </div>
          ...
          <div
            ...
            style={{ animationDelay: '0.15s', maxHeight: '500px' }}
          >
            {/* Header */}
        ...
        <div
          className="nofx-glass p-6 animate-slide-in relative overflow-hidden group mb-6"
          ...
          <div className="flex items-center justify-between mb-5 relative z-10">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded text-xs font-semibold transition-all hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed bg-blue-500/10 text-blue-400 border border-blue-500/30 hover:bg-blue-500/20 hover:border-blue-500/50"
                title={language === 'zh' ? '从交易所同步最新持仓数据（如果在OKX手动平仓过，点击这里刷新）' : 'Sync positions from exchange (refresh if you manually closed positions on OKX)'}
              >
                {syncingPositions ? (
                  <>
                    <Loader2 className="w-3 h-3 animate-spin" />
        ...
        {selectedTraderId && (
          ...
          >
            <ReflectionPanel traderID={selectedTraderId} language={language} maxItems={5} />
          </div>
        )}
