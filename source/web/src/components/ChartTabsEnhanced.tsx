...
interface ChartTabsEnhancedProps {
  traderId: string
...
export function ChartTabsEnhanced({
  traderId,
  selectedSymbol,
  updateKey,
  exchangeId,
}: ChartTabsEnhancedProps) {
  const { language } = useLanguage()
  const [activeTab, setActiveTab] = useState<ChartTab>('kline') // 默认使用K线图表
  ...
  useEffect(() => {
    if (selectedSymbol) {
      console.log(
        '[ChartTabsEnhanced] 收到币种选择:',
        selectedSymbol,
        'updateKey:',
        updateKey
      )
      setChartSymbol(selectedSymbol)
      setActiveTab('kline') // 使用K线图表
    }
  ...
  console.log('[ChartTabsEnhanced] rendering, activeTab:', activeTab)

  return (
    <div
      className={`relative rounded-xl w-full flex flex-col transition-all duration-300 overflow-hidden ${
    ...
    >
      {/* Premium Toolbar */}
      <div
        className="relative z-20 flex flex-wrap md:flex-nowrap items-center justify-between gap-y-2 px-4 py-3 shrink-0"
        style={{
          background: 'linear-gradient(180deg, rgba(20, 24, 32, 0.9) 0%, rgba(15, 18, 25, 0.8) 100%)',
          borderBottom: '1px solid rgba(255, 255, 255, 0.06)',
        }}
      >
        {/* Left: Tab Switcher */}
        <div className="flex items-center gap-1 p-1 rounded-lg" style={{ background: 'rgba(0, 0, 0, 0.3)' }}>
          ...
          >
            ...
            <span className="md:hidden">净值</span>
          </button>
          ...
          >
            ...
            <span className="md:hidden">K线</span>
          </button>

          {/* Market Type Pills - Only when kline active, HIDDEN on mobile */}
          {activeTab === 'kline' && (
      ...
      <div 
        ...
        style={{
          background: 'linear-gradient(180deg, rgba(8, 10, 15, 0.6) 0%, rgba(5, 7, 12, 0.8) 100%)',
        }}
      >
