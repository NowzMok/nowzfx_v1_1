...
func (pb *PositionBuilder) handleOpen(
	...
	if existing == nil {
		// Create new position
		// Use a unique ExchangePositionID that won't conflict with existing positions
		// Format: sync_<symbol>_<side>_<timestamp>_<random> to ensure uniqueness
		exchangePositionID := fmt.Sprintf("sync_%s_%s_%d_%d", symbol, side, tradeTimeMs, time.Now().UnixNano()%1000000)
		...
		logger.Infof("  🏗️ Creating new position: Symbol=%s, Side=%s, Qty=%.6f, Price=%.6f, ExchangePosID=%s",
			symbol, side, quantity, price, exchangePositionID)

		err := pb.positionStore.CreateOpenPosition(position)
		if err != nil {
			logger.Errorf("  ❌ Failed to create position: %v", err)
			return err
		}

		logger.Infof("  ✅ Position created successfully")
		return nil
	}
