...
func (t *FuturesTrader) GetBalance() (map[string]interface{}, error) {
	...
	if err != nil {
		if ctx.Err() == context.DeadlineExceeded {
			logger.Errorf("❌ Binance API timeout after 30s")
			return nil, fmt.Errorf("API request timeout: %w", err)
		}
...
func (t *FuturesTrader) GetPositions() ([]map[string]interface{}, error) {
	...
	if err != nil {
		if ctx.Err() == context.DeadlineExceeded {
			return nil, fmt.Errorf("API request timeout after 30s")
		}
...
// Leverage safety limits by asset type
const (
	MaxLeverageBTCETH  = 20 // Maximum leverage for BTC and ETH
	MaxLeverageAltcoin = 10 // Maximum leverage for altcoins
)
...
func (t *FuturesTrader) SetLeverage(symbol string, leverage int) error {
	// Enforce leverage safety limits
	maxLev := MaxLeverageAltcoin
	if isBTCOrETH(symbol) {
		maxLev = MaxLeverageBTCETH
	}
	if leverage > maxLev {
		logger.Warnf("  ⚠️ Requested leverage %dx exceeds safety limit %dx for %s, capping to %dx", leverage, maxLev, symbol, maxLev)
		leverage = maxLev
	}
	if leverage < 1 {
		leverage = 1
