...
// TraderConfigCache caches trader configurations to reduce DB queries
type TraderConfigCache struct {
	configs   map[string]*traderConfigData // key: userID
	timestamp map[string]time.Time         // key: userID
	mu        sync.RWMutex
...
func NewTraderManager() *TraderManager {
	return &TraderManager{
		...
		configCache: &TraderConfigCache{
			configs:   make(map[string]*traderConfigData),
			timestamp: make(map[string]time.Time),
			cacheTTL:  5 * time.Minute, // 5分钟缓存过期
		},
	}
}
...
func (tm *TraderManager) LoadUserTradersFromStore(st *store.Store, userID string) error {
	...
	var aiModels []*store.AIModel
	var exchanges []*store.Exchange
	var err error

	tm.configCache.mu.RLock()
	cachedData, exists := tm.configCache.configs[userID]
	cachedTime, hasTime := tm.configCache.timestamp[userID]
	tm.configCache.mu.RUnlock()

	// 检查缓存是否有效
	if exists && hasTime && time.Since(cachedTime) < tm.configCache.cacheTTL {
		// 使用缓存数据
		traders = cachedData.traders
		aiModels = cachedData.aiModels
		exchanges = cachedData.exchanges
		logger.Debugf("✅ Using cached trader configurations for user %s (%d traders)", userID, len(traders))
	} else {
		// 缓存过期或不存在，从数据库加载
		...
		// 更新缓存
		tm.configCache.mu.Lock()
		tm.configCache.configs[userID] = &traderConfigData{
			traders:   traders,
			aiModels:  aiModels,
			exchanges: exchanges,
		}
		tm.configCache.timestamp[userID] = time.Now()
		tm.configCache.mu.Unlock()
	}
