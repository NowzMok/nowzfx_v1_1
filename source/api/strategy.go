...
func (s *Server) handleUpdateStrategy(c *gin.Context) {
	...
	// 🔍 验证TriggerPriceConfig是否正确序列化
	if req.Config.TriggerPriceConfig == nil {
		warnings = append(warnings, "TriggerPriceConfig is not set - will use default 'swing' configuration")
		logger.Warnf("⚠️ TriggerPriceConfig validation: nil detected")
	} else {
		logger.Infof("✅ TriggerPriceConfig validation: OK")
	}
	...
	// 🔍 验证序列化后的JSON
	logger.Infof("🔧 [API] Serialized config JSON length: %d bytes", len(configJSON))
	...
	logger.Infof("✅ Strategy updated successfully in database")
