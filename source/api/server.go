...
// klineCache stores cached kline data with expiration time
type klineCache struct {
	data      []market.Kline
	timestamp time.Time
}

// KlineCache manages kline data caching with TTL
type KlineCache struct {
	cache    map[string]*klineCache
	mu       sync.RWMutex
	cacheTTL time.Duration // Cache time-to-live (default 1 minute)
}
...
type Server struct {
	...
	reflectionScheduler *backtest.ReflectionScheduler
	store               *store.Store
...
func NewServer(traderManager *manager.TraderManager, st *store.Store, cryptoService *crypto.CryptoService, backtestManager *backtest.Manager, reflectionScheduler *backtest.ReflectionScheduler, port int) *Server {
	...
	s := &Server{
		...
		port:                port,
		klineCache: &KlineCache{
			cache:    make(map[string]*klineCache),
			cacheTTL: 1 * time.Minute, // Cache kline data for 1 minute
		},
	}
...
// GetRouter returns the Gin router for external route registration
...
func (s *Server) setupRoutes() {
	...
	api := s.router.Group("/api")
	{
		...
		// WebSocket health check (public, no authentication)
		api.GET("/health/websocket", func(c *gin.Context) {
		...
		protected := api.Group("/", s.authMiddleware())
		{
			...
			protected.POST("/traders/:id/sync-positions", s.handleSyncPositions)
			protected.POST("/traders/:id/sync-orders", s.handleSyncOrders)
			protected.POST("/traders/:id/close-position", s.handleClosePosition)
			protected.POST("/traders/:id/modify-tp-sl", s.handleModifyTPSL)
			...
			protected.GET("/pending-orders", s.handlePendingOrders)            // Pending orders from delay execution
			protected.GET("/traders/:id/tpsl-records", s.handleGetTPSLRecords) // TP/SL tracking records
			...
			// Order statistics and analysis routes (改进7)
			protected.GET("/order-statistics", s.handleOrderStatistics)
			protected.GET("/order-statistics-trend", s.handleOrderStatisticsTrend)
			protected.GET("/orders-by-symbol", s.handleOrdersBySymbol)

			// Strategy comparison routes (改进9)
			protected.GET("/strategy-comparison", s.handleStrategyComparison)
			protected.GET("/strategy-performance-trend", s.handleStrategyPerformanceTrend)

			// Cleanup stats route (需要认证)
			protected.GET("/cleanup-stats", s.handleGetCleanupStats)

			// Performance monitoring routes (动态止损性能监控深化)
			protected.GET("/performance/metrics", s.handleGetPerformanceMetrics)
			protected.GET("/performance/daily-summary", s.handleGetDailySummaries)
			protected.GET("/performance/hourly-trends", s.handleGetHourlyTrends)
			protected.GET("/performance/stats", s.handleGetPerformanceStats)
			protected.GET("/performance/alerts", s.handleGetPerformanceAlerts)
			protected.POST("/performance/alerts/:alertId/resolve", s.handleResolveAlert)

			// Backtest routes
			backtest := protected.Group("/backtest")
			s.registerBacktestRoutes(backtest)

			// Order cleanup routes
			orderCleanup := NewOrderCleanupHandlers(s.store)
			orderCleanup.RegisterRoutes(protected)

			// Adaptive Stop Loss routes
			protected.GET("/adaptive-stoploss/:traderId", s.handleGetAdaptiveStopLoss)
			protected.PUT("/adaptive-stoploss/:traderId", s.handleUpdateAdaptiveStopLoss)
		}
	}
}

// handleGetAdaptiveStopLoss Get adaptive stop loss record for a trader
func (s *Server) handleGetAdaptiveStopLoss(c *gin.Context) {
	...
	// Get active records for this trader
	records, err := s.store.AdaptiveStopLoss().GetActiveRecords(traderID)
	if err != nil {
		SafeInternalError(c, "Failed to get adaptive stop loss records", err)
		return
	}

	// ✅ 注意：前端已有过滤逻辑，后端直接返回所有ACTIVE记录
	// 更完善的实现需要查询实际持仓并过滤，但这需要额外的数据库查询
	// 当前依赖前端的openSymbols过滤逻辑

	if len(records) == 0 {
		c.JSON(http.StatusOK, gin.H{
			"exists":  false,
			"message": "No active dynamic stop loss for this trader",
		})
		return
	}
...
// handleUpdateAdaptiveStopLoss Update or create adaptive stop loss record
func (s *Server) handleUpdateAdaptiveStopLoss(c *gin.Context) {
	userID := c.GetString("user_id")
	traderID := c.Param("traderId")
	...
	if req.Enabled {
		// Check if record already exists for this symbol
		existing, err := s.store.AdaptiveStopLoss().GetRecordBySymbol(traderID, req.Symbol)
		if err != nil {
			SafeInternalError(c, "Failed to check existing record", err)
			return
		}
		...
		record := &store.AdaptiveStopLossRecord{
			...
			InitialStopLoss: req.InitialStopLoss,
			CurrentStopLoss: req.InitialStopLoss,
			TakeProfit:      0, // Will be set by strategy
		...
		if existing != nil {
			...
			err = s.store.AdaptiveStopLoss().UpdateRecord(record)
		} else {
			// Create new record
			err = s.store.AdaptiveStopLoss().SaveRecord(record)
		}

		if err != nil {
			SafeInternalError(c, "Failed to save adaptive stop loss", err)
			return
		}

		c.JSON(http.StatusOK, gin.H{
			"message": "Dynamic stop loss enabled",
			"record":  record,
		})
	} else {
		// Disable - close record for this symbol
		if req.Symbol != "" {
			existing, err := s.store.AdaptiveStopLoss().GetRecordBySymbol(traderID, req.Symbol)
			if err == nil && existing != nil {
				err = s.store.AdaptiveStopLoss().CloseRecord(existing.ID)
				if err != nil {
					SafeInternalError(c, "Failed to disable adaptive stop loss", err)
					return
				}
			}
		}
		...
		c.JSON(http.StatusOK, gin.H{
			"message": "Dynamic stop loss disabled",
		})
	}
}
...
func (s *Server) handleCreateTrader(c *gin.Context) {
	...
	// Register trader for reflection scheduling so new traders get analyzed automatically
	if s.reflectionScheduler != nil {
		s.reflectionScheduler.RegisterTrader(traderID)
	}
...
func (s *Server) handleDeleteTrader(c *gin.Context) {
	...
	// Unregister trader from reflection scheduling to avoid processing deleted traders
	if s.reflectionScheduler != nil {
		s.reflectionScheduler.UnregisterTrader(traderID)
	}
...
// handleSyncPositions Sync positions from exchange (refresh positions data)
func (s *Server) handleSyncPositions(c *gin.Context) {
	...
	logger.Infof("🔄 User %s requested position sync for trader %s", userID, traderID)

	// Force refresh positions from exchange
	positions, err := trader.GetPositions()
	if err != nil {
		logger.Infof("❌ Failed to sync positions: %v", err)
		SafeInternalError(c, "Failed to sync positions", err)
		return
	}

	logger.Infof("✅ Position sync completed: %d positions found", len(positions))
...
// handleSyncOrders Sync orders from exchange (refresh orders data)
func (s *Server) handleSyncOrders(c *gin.Context) {
	userID := c.GetString("user_id")
	traderID := c.Param("id")
	...
	logger.Infof("🔄 User %s requested order sync for trader %s", userID, traderID)

	// Force refresh orders from exchange
	// Get all orders (open and closed)
	orders, err := s.store.Order().GetTraderOrdersFiltered(traderID, "", "", 1000)
	if err != nil {
		logger.Infof("❌ Failed to sync orders: %v", err)
		SafeInternalError(c, "Failed to sync orders", err)
		return
	}
	...
	logger.Infof("✅ Order sync completed: %d total orders, %d filled", len(orders), filledCount)
...
func (s *Server) handleClosePosition(c *gin.Context) {
	...
	// 关闭对应的ASL记录
	if err := s.store.AdaptiveStopLoss().CloseRecordBySymbol(traderID, req.Symbol); err != nil {
		logger.Warnf("⚠️ Failed to close ASL record for %s/%s: %v", traderID, req.Symbol, err)
	} else {
		logger.Infof("✅ ASL record closed for %s/%s", traderID, req.Symbol)
	}
...
// handlePendingOrders Get pending orders from delay execution architecture
func (s *Server) handlePendingOrders(c *gin.Context) {
	userID := c.GetString("user_id")
	traderID := c.Query("trader_id")

	// If no trader_id specified, get first trader for this user
	if traderID == "" {
		traders, err := s.store.Trader().List(userID)
	...
	// Get pending orders from database using AnalysisStore
	analysisStore := s.store.Analysis()
	pendingOrders, err := analysisStore.GetPendingOrdersByTrader(traderID)
	if err != nil {
...
func (s *Server) handleKlines(c *gin.Context) {
	...
	// Check cache first
	s.klineCache.mu.RLock()
	cached, exists := s.klineCache.cache[cacheKey]
	s.klineCache.mu.RUnlock()

	if exists && time.Since(cached.timestamp) < s.klineCache.cacheTTL {
		// Cache hit - return cached data
		logger.Debugf("✅ Using cached kline data for %s", cacheKey)
	...
	s.klineCache.mu.Lock()
	s.klineCache.cache[cacheKey] = &klineCache{
		data:      klines,
		timestamp: time.Now(),
	}
	s.klineCache.mu.Unlock()

	logger.Debugf("📊 Cached kline data for %s (TTL: %v)", cacheKey, s.klineCache.cacheTTL)
...
// handleErrorStats Error statistics
func (s *Server) handleErrorStats(c *gin.Context) {
	...
	// 使用API handler中的GetErrorStats
	handler := GetErrorStats(trader)
	handler(c)
}

// handleRecentErrors Recent errors
func (s *Server) handleRecentErrors(c *gin.Context) {
	...
	handler := GetRecentErrors(trader)
	handler(c)
}

// handleErrorReport Error report
func (s *Server) handleErrorReport(c *gin.Context) {
	_, traderID, err := s.getTraderFromQuery(c)
	if err != nil {
		SafeBadRequest(c, "Invalid trader ID")
		return
	...
	handler := GetErrorReport(trader)
	handler(c)
}

// handleErrorRate Error rate
func (s *Server) handleErrorRate(c *gin.Context) {
	_, traderID, err := s.getTraderFromQuery(c)
	if err != nil {
		SafeBadRequest(c, "Invalid trader ID")
		return
	...
	handler := GetErrorRate(trader)
	handler(c)
}

// handleClearErrors Clear errors
func (s *Server) handleClearErrors(c *gin.Context) {
	_, traderID, err := s.getTraderFromQuery(c)
	if err != nil {
		SafeBadRequest(c, "Invalid trader ID")
		return
